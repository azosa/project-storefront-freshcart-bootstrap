...
export class StoreProductsComponent {
  readonly search: FormGroup = new FormGroup({ searchMarket: new FormControl() });

  
    readonly store$: Observable<StoreModel[]|any> = this._activatedRoute.params.pipe(
      switchMap((data) => {
  ...
  .pipe(
    ...
    ([products,search,store]) =>
      search.searchMarket ? products.filter((prod) => 
      prod.name.toLowerCase().includes(search.searchMarket.toLowerCase())&&
